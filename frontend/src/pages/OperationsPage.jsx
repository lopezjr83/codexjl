...
export default function OperationsPage() {
  ...
  const categoryOptions = [
    { key: 'visitor', label: 'Visita' },
    { key: 'client', label: 'Cliente' },
    { key: 'provider', label: 'Proveedor' }
  ];

  const onDocumentPhotoChange = (event) => {
    const file = event.target.files?.[0];
    if (!file) return;
    if (!file.type.startsWith('image/')) {
      setError('Solo se permiten imágenes para el documento.');
      return;
    }
    ...
    reader.onload = () => {
      ...
      setForm((prev) => ({ ...prev, visitorDocumentPhoto: base64 }));
      setError('');
    };
        ...
        <section className="grid-panels">
          <section className="panel">
            ...
            <form className="grid-form" onSubmit={submit}>
              <div className="category-toggle" role="radiogroup" aria-label="Tipo de ingreso">
                {categoryOptions.map((option) => (
                  <button
                    key={option.key}
                    type="button"
                    className={form.category === option.key ? 'active' : ''}
                    onClick={() => setForm({ ...form, category: option.key })}
                    aria-pressed={form.category === option.key}
                  >
                    {option.label}
                  </button>
                ))}
              ...
              <div className="document-photo-field">
                <label>Foto del documento (opcional, máx 2MB)</label>
                <input className="file-input" type="file" accept="image/*" onChange={onDocumentPhotoChange} />
                {form.visitorDocumentPhoto && <small>Foto cargada correctamente.</small>}
              </div>

              {form.category === 'client' && (
                <>
                  <input placeholder="Teléfono" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} required />
                  <input type="number" min="1" max="10" placeholder="Tarjeta cliente (1-10)" value={form.badgeNumber} onChange={(e) => setForm({ ...form, badgeNumber: e.target.value })} required />
                </>
              )}
              ...
              {form.category === 'provider' && (
                <>
                  ...
                  <input type="number" min="1" max="10" placeholder="Tarjeta proveedor (1-10)" value={form.badgeNumber} onChange={(e) => setForm({ ...form, badgeNumber: e.target.value })} required />
                </>
              )}
