...
Ahora también está incluido el workflow `.github/workflows/vercel-deploy.yml` para desplegar automáticamente **frontend** y **backend** en Vercel cuando haces push a `main` o `work`.
...
Con eso, cada push a `main`/`work` ejecutará:
1. `vercel pull` (entorno de producción),
2. `vercel build`,
3. `vercel deploy --prebuilt --prod`

para ambos proyectos (frontend y backend).
